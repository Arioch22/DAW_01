...
JOIN RESERVA r on
	r.Clase = c.Codigo_Clase;
/*GROUP BY mc.Codigo_Monitor, mc.Cobra */
/*HAVING
	COUNT(r.Usuario) >4;*/
...
SELECT
	...
	mc.Codigo_Monitor,
	ct.Descripcion,
	SUM (DISTINCT mc.Cobra) AS Salario_Clase
From 
MONITOR_CLASES mc 
JOIN MONITOR m ON 
	m.Codigo_Monitor = mc.Codigo_Monitor
...
JOIN CLASE_TIPO ct ON 
	ct.CLASE_TIPO = c.id_Tipo
right JOIN RESERVA r ON 
	r.Clase =c.Codigo_Clase
GROUP BY mc.Codigo_Monitor, c.Codigo_Clase 
Having COUNT(r.Usuario) > 4 ;
